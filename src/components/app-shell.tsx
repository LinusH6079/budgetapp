...
export function AppShell({ children, userName, householdName }: AppShellProps) {
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 pb-24 pt-4 sm:px-6 lg:px-8">
      <header className="app-panel sticky top-4 z-20 mb-6 overflow-hidden px-4 py-4 sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-[var(--color-muted)]">Budgetkompis</p>
            <h1 className="mt-1 text-2xl font-semibold tracking-[-0.03em]">{householdName || "Ditt hushåll"}</h1>
            <p className="muted mt-1">Inloggad som {userName}</p>
          </div>

          <form action={logoutAction}>
            <button className="rounded-full bg-[var(--color-ink)] px-5 py-3 text-sm font-semibold text-white">
              Logga ut
            </button>
          </form>
        </div>
